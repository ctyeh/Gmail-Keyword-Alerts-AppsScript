 ...
 * - core/gmail.js (hasLabel, addLabel, isFromExcludedDomain, checkKeywords, extractActualContent, isForwardedEmail)
 * - core/gemini.js (analyzeEmailWithGemini, logEmailAnalysisResult)
 * - core/slack.js (sendNotification)
 * - modules/emotionStorage.js (storeEmotionAnalysisResult)
 * - modules/ignoreRules.js (shouldIgnore)
 * - modules/notificationRules.js (shouldNotify)
 */
...
function processThreads(threads) {
  ...
  for (const thread of threads) {
    ...
    for (const message of messages) {
      if (!gmail.hasLabel(message, CHECKED_LABEL)) {
...
function processMessage(message, subject) {
  ...
  // 檢查是否應該完全忽略此郵件
  if (ignoreRules.shouldIgnore(message, subject, body)) {
  ...
  // 檢查寄件者是否來自排除的網域
  const isExcludedDomain = gmail.isFromExcludedDomain(from);
  if (isExcludedDomain) {
    Logger.log(`郵件來自排除網域: ${from}, 主旨: ${subject}`);
  }
  
  // 檢查是否為轉寄郵件
  const isForwarded = gmail.isForwardedEmail(subject);
  if (isForwarded) {
    Logger.log(`檢測到轉寄郵件: ${subject}`);
  }
  
  // 獲取郵件的實際內容（排除引用部分）
  const actualBody = gmail.extractActualContent(body);
  
  // 檢查郵件內容是否包含關鍵字
  const foundKeywords = gmail.checkKeywords(subject, actualBody, isForwarded);
  ...
  if (USE_GEMINI_API) {
    ...
    emailAnalysis.aiAnalysisResult = gemini.analyzeEmailWithGemini(subject, actualBody, from);
    
    // 將情緒分析結果存儲到 Properties 服務
    if (emailAnalysis.aiAnalysisResult) {
      emotionStorage.storeEmotionAnalysisResult(message, emailAnalysis.aiAnalysisResult);
  ...
  // 記錄郵件分析結果到日誌
  gemini.logEmailAnalysisResult(message, subject, from, foundKeywords, emailAnalysis.aiAnalysisResult);
  
  // 處理標籤和通知
  
  // 標記為已檢查（所有經過分析的郵件都會被標記）
  gmail.addLabel(message, CHECKED_LABEL);
  
  // 檢查是否符合關鍵字，標記
  if (emailAnalysis.keywordsFound.length > 0) {
    gmail.addLabel(message, KEYWORD_LABEL);
  ...
  if (emailAnalysis.aiDetected) {
    ...
    if (!isExcludedDomain) {
      gmail.addLabel(message, AI_ALERT_LABEL);
  ...
  // 決定是否發送通知
  const shouldSendNotification = notificationRules.shouldNotify(emailAnalysis, message, subject, actualBody);
  
  // 發送通知
  if (shouldSendNotification) {
    try {
      slack.sendNotification(subject, from, date, body, actualBody, link, emailAnalysis, message);
